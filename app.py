...
#DB CRUD OPERATIONS
@app.route('/playlistCRUD', methods = ['GET'])
def get_playlist():
...
@app.route('/playlistCRUD', methods=['POST'])
def addSong():
...
@app.route('/playlistCRUD/<id>', methods=['PUT'])
def update_song(id):
	song = request.get_json()
	session['playlist'][id] = song
	return jsonify(session['playlist'][id]), 200

@app.route('/playlistCRUD/<id>', methods=['DELETE'])
def delete_song(id):
...
def downloadLink(link):
	...
	secure_filepath = os.path.join(app.config['upload_folder'], secure_fileName)
